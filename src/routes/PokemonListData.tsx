...
function PokemonListData() {
  ...
  const getPokemonSprite = useCallback(
    (pokemon: PokemonSummary | null | undefined) => {
      if (!pokemon) return '';
      return (
    ...
    Promise.all(
      missing.map(async ({ name, url }) => {
        try {
          ...
          if (!sprite) {
            try {
              const speciesData = await fetchByUrl(`${SPECIES_URL_PREFIX}${name}`).unwrap();
  ...
  const inkStrong = isRainbow ? '#1f2937' : isLightColor(theme.bg1) ? '#1f2937' : '#f8fafc';
  ...
  const tipIndex = useMemo(
    () => now.getMinutes() % greetingTips.length,
    [now, greetingTips.length]
  );
  ...
  return (
    <div
      ...
      style={{
        ...
        ['--theme-nav' as any]: selectedTypes.length === 0 ? '#7aa7ff' : theme.accent,
        ['--theme-nav-ink' as any]: selectedTypes.length === 0 ? '#1f2a44' : accentInk,
            ...
            return (
              ...
              >
                ...
                <span className={classes.filterTypeIconWrap} aria-hidden="true">
                  {getTypeIcon(name) ? (
                    <img className={classes.filterTypeIcon} src={getTypeIcon(name)} alt="" />
                  ) : null}
                </span>
        ...
        <section className={`${classes.mainColumn} ${classes.mainLayer}`}>
          <div className={classes.featuredRow}>
            ...
            >
              {featuredPokemon ? (
                <div
                  className={`${classes.pokeFeaturedContent} ${classes.pokeFeaturedContentCarousel}`}
                >
                  <h2 className={`${classes.pokeCardTitle} ${classes.pokeFeaturedTitle}`}>
                    Featured
                  </h2>
